...
2. 对峙位：开场期间人物/怪物都已在对峙坐标。两人时与平时站位相同——一个在 `player1` 中心、一个在侧槽。敌人卡相对父物体 localScale 与人物 `PlayerItem` 相同（0.9）；侧槽父节点 0.85 所以看起来更小，中心仍是 0.9
3. `VS` 飞入居中并留着；随后对话开始
...
通关商店货架 / 已购见 [`ShopItem.md`](../../Item/ShopItem.md)，买卖在 [`ShopDetail.md`](../Popup/ShopDetail.md)。  
...
- `player1` 是中心：只剩 1 个敌人就站中间；2 个时第一个开牌的站中间；3 个拼牌时当前对手站中间。换位时人物卡从旧槽滑到新槽，不是瞬移。
