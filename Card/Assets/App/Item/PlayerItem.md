...
| `TopDialog` | 开场对白气泡（卡上方）；敌人开场用。默认隐藏。子节点 `DialogText` |
| `BottomDialog` | 开场对白气泡（卡下方，预制体已翻转）；人物开场用。默认隐藏。子节点 `DialogText` |
...
item.PlayDialog(top: true, "站住！");   // TopDialog：敌人开场
item.PlayDialog(top: false, "放马过来。"); // BottomDialog：人物开场
item.HideDialog();                      // 渐隐后关掉当前气泡
item.HideDialogImmediate();             // 立刻关掉，打断 tween
```

`RootRect` / `RootAnimator` 给 [`AttackCutscene.md`](../UI/Game/AttackCutscene.md) 做冲锋演出。开场对白由 [`OpeningCutscene`](../UI/Game/OpeningCutscene.cs) 调 `PlayDialog`：先对峙位移（人物上、怪物下）并出 VS，再中间敌人先说、玩家回，其余从左到右各说一句，对峙结束回位后发牌。
