...
1. 该敌人从 5 张里选出不超过本关 `MonsterCardHandScoreLevelLimit` 的最大 3 张并翻面亮牌（未选中的 2 张保持背面），然后双方比牌。按 `HandScoreConfig.Level` 比大小：豹子 ＞ 顺金 ＞ 顺子 ＞ 金花 ＞ 对子 ＞ 散牌。同牌型比点数。玩家 vs 敌人时平局算玩家赢。老花眼、错峰出行、235 只改玩家牌型，敌人不吃这些遗物。升职改写玩家实际牌型（展示、倍率、比牌一起升）；降低只让敌人比牌顺位 -1，展示和出伤仍按原牌型。细则见 [`RelicMechanics.md`](RelicMechanics.md)。
