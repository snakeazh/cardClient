...
1. 该敌人从 5 张里选出最大的 3 张并朝玩家方向移开，然后双方亮牌。按炸金花比大小：豹子 ＞ 顺金 ＞ 金花 ＞ 顺子 ＞ 对子 ＞ 散牌。同牌型比点数。开牌方需 **严格大于** 才算赢，平局视为玩家输。
...
攻击力进关时从配置写入 `SeatState.Attack`，本关内不随扣血变化。`BaseChips` 为亮出三张牌的 `ChipValue` 全加（A=11，其余为点数；K=13、Q=12、J=11）。
...
豹子 > 顺金 > 金花 > 顺子 > 对子 > 散牌。同牌型比点数（比牌 `RankKey`：A=14）。伤害用的 `ChipValue`：A=11，其余为点数。这是牌力数值，不是货币。
...
BOSS 关随机词缀（禁搓花色、禁计分、燧石、锋芒、透视眼等）。
