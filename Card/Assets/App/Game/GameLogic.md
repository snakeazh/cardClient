...
- 玩家点选手牌，选中的牌上移；必须选满 **3** 张才能开牌。选满后 HUD 立刻显示这 3 张的牌型作为提示。开牌只用这 3 张，剩余 2 张不参与比牌。
- 结算时敌人从 5 张里枚举 `C(5,3)` 种组合，选出不超过本关 `LevelConfig.MonsterCardHandScoreLevelLimit` 的最大牌型 3 张并翻面亮牌（不抬起）；全部超限则取 Level 最低的一组。未选中的 2 张不翻牌、不参与比牌、不播赢家结算动画。透视时该座位 5 张全部选中：先抬起，抬完再背面透视（不翻牌）；开牌结算会先清选中，再锁这 3 张。
...
| ChaKanGood | 透视：点选一名敌人，5 张全部选中并先抬起，抬完再背面透视（不翻牌）；不能透视自己的牌；开牌时先重置，再锁不超过本关上限的最大 3 张 |
...
1. 该敌人从 5 张里选出不超过本关 `MonsterCardHandScoreLevelLimit` 的最大 3 张并翻面亮牌（未选中的 2 张保持背面），然后双方比牌。按 `HandScoreConfig.Level` 比大小：豹子 ＞ 顺金 ＞ 顺子 ＞ 金花 ＞ 对子 ＞ 散牌。同牌型比点数。玩家 vs 敌人时平局算玩家赢。老花眼、错峰出行、235 只改玩家牌型，敌人不吃这些遗物。升职改写玩家实际牌型（展示、倍率、比牌一起升）；降低改写敌人实际牌型（展示、倍率、比牌一起降），散牌为下限。细则见 [`RelicMechanics.md`](RelicMechanics.md)。
...
击杀本关全部敌人 → 发本关 `LevelConfig.GetGold`（可乘经济教授）+ 击杀数 × `KillMonsterGetGold` 为**局内金币**进商店；本手伤害换金已在比牌结束时入账。[`BattleSettleUpPop.md`](../UI/Popup/BattleSettleUpPop.md) 只展示本关 `Stage` 积分和本关发放的金币，不展示本章节累计。提现 / 双倍 / 商店出售飞币见 [`CoinFlyFx.md`](../UI/Effects/CoinFlyFx.md)。
